...
ValCycle, ValEpisode = cfg.valParams()

RecordGradient, GradientUpdateCycle = cfg.gradRecParams()

TrainingVerbose, UseVisdom = cfg.verboseParams()

types, titles, xlabels, ylabels, legends = cfg.plotParams()
...
train_path_manager = PathManager(
    dataset=data_folder, d_type="train", model_name=model_name, version=version
)
val_path_manager = PathManager(
    dataset=data_folder, d_type="validate", model_name=model_name, version=version
)

train_dataset = SeqFileDataset(
    train_path_manager.FileData(), train_path_manager.FileSeqLen(), N
)
val_dataset = SeqFileDataset(
    val_path_manager.FileData(), val_path_manager.FileSeqLen(), N
)
...
stat = TrainStatManager(
    model_save_path=train_path_manager.Model(),
...
if modelParams["usePretrained"]:
    word_matrix = t.Tensor(
        np.load(train_path_manager.WordEmbedMatrix(), allow_pickle=True)
    )
else:
    word_matrix = None
...
if model_type == "ProtoNet":
    ...
    model = MetaSGD(n=n, loss_fn=loss, pretrained_matrix=word_matrix, **modelParams)
elif model_type == "ATAML":
    model = ATAML(n=n, loss_fn=loss, pretrained_matrix=word_matrix, **modelParams)
elif model_type == "HybridAttentionNet":
    model = HAPNet(k=k, pretrained_matrix=word_matrix, **modelParams)
elif model_type == "ConvProtoNet":
    model = ConvProtoNet(k=k, pretrained_matrix=word_matrix, **modelParams)
elif model_type == "PerLayerATAML":
    model = PerLayerATAML(
        n=n, loss_fn=loss, pretrained_matrix=word_matrix, **modelParams
    )
elif model_type == "Reptile":
    model = Reptile(n=n, loss_fn=loss, pretrained_matrix=word_matrix, **modelParams)
elif model_type == "TCProtoNet":
    model = TCProtoNet(pretrained_matrix=word_matrix, **modelParams)
elif model_type == "FEAT":
    model = FEAT(pretrained_matrix=word_matrix, **modelParams)
elif model_type == "AFEAT":
    model = AFEAT(pretrained_matrix=word_matrix, **modelParams)
elif model_type == "NnNet":
    model = NnNet(pretrained_matrix=word_matrix, **modelParams)
elif model_type == "IMP":
    model = IMP(pretrained_matrix=word_matrix, **modelParams)
elif model_type == "SIMPLE":
    model = SIMPLE(pretrained_matrix=word_matrix, **modelParams)
elif model_type == "HybridIMP":
    model = HybridIMP(pretrained_matrix=word_matrix, **modelParams)
...
for name, par in model.named_parameters():
    ...
    if name in lrs:
        ...
        parameters += [
            {"params": [par], "lr": default_lr, "weight_decay": weight_decay}
        ]
...
if optimizer_type == "adam":
    ...
    optimizer = t.optim.Adagrad(
        model.parameters(), lr=default_lr, weight_decay=weight_decay
    )
...
scheduler = t.optim.lr_scheduler.StepLR(
    optimizer, step_size=LRDecayIters, gamma=LRDecayGamma
)
...
saveRunVersionConfig(
    cur_v=version,
    dataset=data_folder,
    model=model_type,
    cfg={"k": k, "n": n, "qk": qk, "desc": cfg.desc()},
)
...
with t.autograd.set_detect_anomaly(False):
    for epoch in range(TrainingEpoch):
        ...
        if model_type == "TCProtoNet":
            acc_val, loss_val_item = penalQLossProcedure(
                model, taskBatchSize, train_task, loss, optimizer, scheduler, train=True
            )

        elif model_type == "FEAT":
            acc_val, loss_val_item = featProcedure(
                model,
                n,
                k,
                qk,
            ...
            acc_val, loss_val_item = impProcedure(
                model,
                taskBatchSize,
                train_task,
                optimizer,
            ...
            acc_val, loss_val_item = queryLossProcedure(
                model, taskBatchSize, train_task, loss, optimizer, scheduler, train=True
            )
        ...
        # 记录任务batch的平均正确率和损失值
        stat.recordTraining(acc_val / taskBatchSize, loss_val_item / taskBatchSize)
        ...
        if (epoch + 1) % ValCycle == 0:
            ...
            for test_i in range(ValEpisode):
                if model_type == "TCProtoNet":
                    validate_acc_oneiter, validate_loss_oneiter = penalQLossProcedure(
                        model, 1, val_task, loss, None, None, train=False
                    )
                    validate_acc += validate_acc_oneiter
                    validate_loss += validate_loss_oneiter

                elif model_type == "FEAT":
                    validate_acc_oneiter, validate_loss_oneiter = featProcedure(
                        model,
                        n,
                        k,
                        qk,
                    ...
                    validate_acc_oneiter, validate_loss_oneiter = impProcedure(
                        model, 1, val_task, None, None, train=False, acc_only=True
                    )
                    validate_acc += validate_acc_oneiter
                    validate_loss += validate_loss_oneiter

                else:
                    validate_acc_oneiter, validate_loss_oneiter = queryLossProcedure(
                        model, 1, val_task, loss, None, None, train=False
                    )
                    validate_acc += validate_acc_oneiter
                    validate_loss += validate_loss_oneiter
